...
def f_graph_weight(w_type, w_min, w_max, num_edges):
    ...
    elif(isinstance(float(w_type), float)):
        arr_weight = arr_weight + min(w_max, float(w_type))
...
def f_creat_networks():
    ...
    for n in range(num_networks):
        ...
        for r in range(DICT_NETWORK["NUM_REP"]):
            ...
            df_graph_edgelist["weight"] = f_graph_weight(g_w_type, g_w_min, g_w_max, df_graph_edgelist.shape[0])
